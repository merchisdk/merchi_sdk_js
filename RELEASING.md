...
In npm package **merchi_sdk_js** settings, add a GitHub Actions trusted publisher:

- Owner: `merchisdk`
- Repository: `merchi_sdk_js`
...
- Installs dependencies and runs the source package tests before publishing. npm install also repairs the historical lockfile/manifest mismatch; the resulting lockfile and version are committed together.
...
This JavaScript SDK ships src directly and does not require a compiled build or the wallet component packages. Consumers such as Dashboard retain their normal lockfile update/review process; this workflow does not merge or deploy consumers.
